...
> ela garante integridade de graça. Referência por ID é decisão de *domínio*; FK é
> decisão de *persistência*. As duas coexistem sem contradição.
...
| Agendar/editar paciente anonimizado | 422 `BUSINESS_RULE_VIOLATION` | "Paciente anonimizado (LGPD) não pode receber novos agendamentos." |
...
- **"Excluir agendamento" significa cancelar.** O verbo HTTP continua `DELETE`, o efeito é `status = CANCELLED`. Exclusão física destruiria a trilha e colidiria com o direito ao esquecimento (que exige *preservar* o histórico de atendimento).
...
| 02.02 | F2 (2/2) | Rotas protegidas: `JwtAuthGuard` global, `@Public()`, `@CurrentDoctor()`, `refresh`, `logout`, `me` | — | ⬜ |
